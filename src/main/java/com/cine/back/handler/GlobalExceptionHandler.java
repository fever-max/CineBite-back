...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> handleNoSuchElementException(NoSuchElementException e) {
        return ResponseEntity.status(NOT_FOUND)
                .body("데이터를 찾을 수 없음 - " + e.getMessage());
    }

    @ExceptionHandler(EntityNotFoundException.class)
    public ResponseEntity<String> handleEntityNotFoundException(EntityNotFoundException e) {
        return ResponseEntity.status(NOT_FOUND)
                .body("엔티티를 찾을 수 없음 - " + e.getMessage());
    }

    @ExceptionHandler(EvaluationNotPermittedException.class)
    public ResponseEntity<String> handleEvaluationNotPermittedException(EvaluationNotPermittedException e) {
        return ResponseEntity.status(CONFLICT)
                .body("아직 평가할 수 없음 - " + e.getMessage());
    }
    ...
    public ResponseEntity<String> handleAlreadyEvaluatedException(AlreadyEvaluatedException e) {
        return ResponseEntity.status(CONFLICT)
                .body(e.getMessage());
    }
    ...
    public ResponseEntity<String> handleAddFavoriteFailure(handleAddFavoriteFailure e) {
        return ResponseEntity.status(BAD_REQUEST)
                .body(e.getMessage());
    }

    @ExceptionHandler(EvaluationNotFoundException.class)
    public ResponseEntity<String> handleEvaluationNotFoundException(EvaluationNotFoundException e) {
        return ResponseEntity.status(NOT_FOUND)
                .body(e.getMessage());
    }
}
