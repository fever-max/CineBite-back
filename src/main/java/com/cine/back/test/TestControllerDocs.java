...
public interface TestControllerDocs {
        ...
        @Operation(summary = "파일 저장", description = "파일을 저장합니다.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "파일 저장 성공"),
                        @ApiResponse(responseCode = "400", description = "파일 저장 실패") })
        public ResponseEntity<String> uploadFile(@RequestParam("file") MultipartFile imgFile,
                        TestFileEntity fileEntity);

        @Operation(summary = "파일 정보 반환", description = "파일 전체 정보를 반환합니다.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "파일 전체 정보 반환 성공"),
                        @ApiResponse(responseCode = "400", description = "파일 전체 정보 반환 실패") })
